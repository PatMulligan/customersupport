...
async def create_merchant(user_id: str, m: PartialMerchant) -> Merchant:
    ...
    await db.execute(
        """
        INSERT INTO customersupport.merchants (user_id, id, private_key, public_key, meta)
        VALUES (?, ?, ?, ?, ?)
        """,
...
async def update_merchant(
    ...
    await db.execute(
        f"""
            UPDATE customersupport.merchants SET meta = ?, time = {db.timestamp_now}
...
async def touch_merchant(user_id: str, merchant_id: str) -> Optional[Merchant]:
    await db.execute(
        f"""
            UPDATE customersupport.merchants SET time = {db.timestamp_now}
            WHERE id = ? AND user_id = ?
        """,
        (merchant_id, user_id),
    )
...
async def get_merchant(user_id: str, merchant_id: str) -> Optional[Merchant]:
    row = await db.fetchone(
        """SELECT * FROM customersupport.merchants WHERE user_id = ? AND id = ?""",
        (
            user_id,
            merchant_id,
        ),
...
async def get_merchant_by_pubkey(public_key: str) -> Optional[Merchant]:
    row = await db.fetchone(
        """SELECT * FROM customersupport.merchants WHERE public_key = ? """,
        (public_key,),
    )
...
async def get_merchants_ids_with_pubkeys() -> List[str]:
    rows = await db.fetchall(
        """SELECT id, public_key FROM customersupport.merchants""",
    )
...
async def get_merchant_for_user(user_id: str) -> Optional[Merchant]:
    row = await db.fetchone(
        """SELECT * FROM customersupport.merchants WHERE user_id = ? """,
        (user_id,),
    )
...
async def delete_merchant(merchant_id: str) -> None:
    await db.execute(
        "DELETE FROM customersupport.merchants WHERE id = ?",
        (merchant_id,),
    )
...
async def create_zone(merchant_id: str, data: PartialZone) -> Zone:
    ...
    await db.execute(
        f"""
        INSERT INTO customersupport.zones (id, merchant_id, name, currency, cost, regions)
        VALUES (?, ?, ?, ?, ?, ?)
        """,
        (
            zone_id,
...
async def update_zone(merchant_id: str, z: Zone) -> Optional[Zone]:
    await db.execute(
        f"UPDATE customersupport.zones SET name = ?, cost = ?, regions = ?  WHERE id = ? AND merchant_id = ?",
...
async def get_zone(merchant_id: str, zone_id: str) -> Optional[Zone]:
    row = await db.fetchone(
        "SELECT * FROM customersupport.zones WHERE merchant_id = ? AND id = ?",
        (
            merchant_id,
            zone_id,
        ),
...
async def get_zones(merchant_id: str) -> List[Zone]:
    rows = await db.fetchall(
        "SELECT * FROM customersupport.zones WHERE merchant_id = ?", (merchant_id,)
    )
...
async def delete_zone(merchant_id: str, zone_id: str) -> None:
    ...
    await db.execute(
        "DELETE FROM customersupport.zones WHERE merchant_id = ? AND id = ?",
        (
            merchant_id,
            zone_id,
        ),
...
async def delete_merchant_zones(merchant_id: str) -> None:
    await db.execute(
        "DELETE FROM customersupport.zones WHERE merchant_id = ?", (merchant_id,)
    )
...
async def create_stall(merchant_id: str, data: PartialStall) -> Stall:
    ...
    await db.execute(
        f"""
        INSERT INTO customersupport.stalls
...
async def get_stall(merchant_id: str, stall_id: str) -> Optional[Stall]:
    row = await db.fetchone(
        "SELECT * FROM customersupport.stalls WHERE merchant_id = ? AND id = ?",
        (
            merchant_id,
            stall_id,
        ),
...
async def get_stalls(merchant_id: str, pending: Optional[bool] = False) -> List[Stall]:
    rows = await db.fetchall(
        "SELECT * FROM customersupport.stalls WHERE merchant_id = ? AND pending = ?",
        (
            merchant_id,
            pending,
        ),
...
async def get_last_stall_update_time() -> int:
    row = await db.fetchone(
        """
            SELECT event_created_at FROM customersupport.stalls 
...
async def update_stall(merchant_id: str, stall: Stall) -> Optional[Stall]:
    await db.execute(
        f"""
            UPDATE customersupport.stalls SET wallet = ?, name = ?, currency = ?, pending = ?, event_id = ?, event_created_at = ?, zones = ?, meta = ?
...
async def delete_stall(merchant_id: str, stall_id: str) -> None:
    await db.execute(
        "DELETE FROM customersupport.stalls WHERE merchant_id =? AND id = ?",
        (
            merchant_id,
            stall_id,
        ),
...
async def delete_merchant_stalls(merchant_id: str) -> None:
    await db.execute(
        "DELETE FROM customersupport.stalls WHERE merchant_id = ?",
        (merchant_id,),
    )
...
async def create_product(merchant_id: str, data: PartialProduct) -> Product:
    ...
    await db.execute(
        f"""
        INSERT INTO customersupport.products 
...
async def update_product(merchant_id: str, product: Product) -> Product:
    ...
    await db.execute(
        f"""
        UPDATE customersupport.products set name = ?, price = ?, quantity = ?,  pending = ?, event_id =?, event_created_at = ?, image_urls = ?, category_list = ?, meta = ?
        WHERE merchant_id = ? AND id = ?
        """,
        (
            product.name,
...
async def update_product_quantity(
    ...
    await db.execute(
        f"UPDATE customersupport.products SET quantity = ?  WHERE id = ?",
        (new_quantity, product_id),
    )
    row = await db.fetchone(
        "SELECT * FROM customersupport.products WHERE id = ?",
        (product_id,),
    )
...
async def get_product(merchant_id: str, product_id: str) -> Optional[Product]:
    row = await db.fetchone(
        "SELECT * FROM customersupport.products WHERE merchant_id =? AND id = ?",
        (
            merchant_id,
            product_id,
        ),
...
async def get_products(
    ...
    rows = await db.fetchall(
        "SELECT * FROM customersupport.products WHERE merchant_id = ? AND stall_id = ? AND pending = ?",
...
async def get_wallet_for_product(product_id: str) -> Optional[str]:
    row = await db.fetchone(
        """
        SELECT s.wallet FROM customersupport.products p
        INNER JOIN customersupport.stalls s
        ON p.stall_id = s.id
...
async def get_last_product_update_time() -> int:
    row = await db.fetchone(
        """
            SELECT event_created_at FROM customersupport.products 
            ORDER BY event_created_at DESC LIMIT 1
        """,
        (),
    )
...
async def delete_product(merchant_id: str, product_id: str) -> None:
    await db.execute(
        "DELETE FROM customersupport.products WHERE merchant_id =? AND id = ?",
        (
            merchant_id,
            product_id,
        ),
...
async def delete_merchant_products(merchant_id: str) -> None:
    await db.execute(
        "DELETE FROM customersupport.products WHERE merchant_id = ?",
        (merchant_id,),
    )
...
async def create_order(merchant_id: str, o: Order) -> Order:
    await db.execute(
        f"""
        INSERT INTO customersupport.orders (
            merchant_id, 
            id, 
            event_id, 
            event_created_at,
...
async def get_order(merchant_id: str, order_id: str) -> Optional[Order]:
    row = await db.fetchone(
        "SELECT * FROM customersupport.orders WHERE merchant_id =? AND id = ?",
        (
            merchant_id,
            order_id,
        ),
...
async def get_order_by_event_id(merchant_id: str, event_id: str) -> Optional[Order]:
    row = await db.fetchone(
        "SELECT * FROM customersupport.orders WHERE merchant_id =? AND  event_id =?",
        (
            merchant_id,
            event_id,
        ),
...
async def get_orders(merchant_id: str, **kwargs) -> List[Order]:
    ...
    rows = await db.fetchall(
        f"SELECT * FROM customersupport.orders WHERE merchant_id = ? {q} ORDER BY event_created_at DESC",
        (merchant_id, *values),
    )
...
async def get_orders_for_stall(
    ...
    if q:
        ...
        values = (v for v in kwargs.values() if v != None)
    rows = await db.fetchall(
        f"SELECT * FROM customersupport.orders WHERE merchant_id = ? AND stall_id = ? {q} ORDER BY time DESC",
...
async def update_order(merchant_id: str, order_id: str, **kwargs) -> Optional[Order]:
    ...
    await db.execute(
        f"""
            UPDATE customersupport.orders SET {q} WHERE merchant_id = ? and id = ?
        """,
...
async def update_order_paid_status(order_id: str, paid: bool) -> Optional[Order]:
    await db.execute(
        f"UPDATE customersupport.orders SET paid = ?  WHERE id = ?",
        (paid, order_id),
    )
    row = await db.fetchone(
        "SELECT * FROM customersupport.orders WHERE id = ?",
        (order_id,),
    )
    return Order.from_row(row) if row else None
...
async def update_order_shipped_status(
    ...
    await db.execute(
        f"UPDATE customersupport.orders SET shipped = ?  WHERE merchant_id = ? AND id = ?",
...
async def delete_merchant_orders(merchant_id: str) -> None:
    await db.execute(
        "DELETE FROM customersupport.orders WHERE merchant_id = ?",
        (merchant_id,),
    )
...
async def create_direct_message(
    ...
    await db.execute(
        f"""
        INSERT INTO customersupport.direct_messages (merchant_id, id, event_id, event_created_at, message, public_key, type, incoming)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(event_id) DO NOTHING
        """,
        (
...
async def get_direct_message(merchant_id: str, dm_id: str) -> Optional[DirectMessage]:
    row = await db.fetchone(
        "SELECT * FROM customersupport.direct_messages WHERE merchant_id = ? AND id = ?",
        (
            merchant_id,
            dm_id,
        ),
...
async def get_direct_message_by_event_id(
    ...
    row = await db.fetchone(
        "SELECT * FROM customersupport.direct_messages WHERE merchant_id = ? AND event_id = ?",
        (
            merchant_id,
            event_id,
        ),
...
async def get_direct_messages(merchant_id: str, public_key: str) -> List[DirectMessage]:
    rows = await db.fetchall(
        "SELECT * FROM customersupport.direct_messages WHERE merchant_id = ? AND public_key = ? ORDER BY event_created_at",
...
async def get_orders_from_direct_messages(merchant_id: str) -> List[DirectMessage]:
    rows = await db.fetchall(
        "SELECT * FROM customersupport.direct_messages WHERE merchant_id = ? AND type >= 0 ORDER BY event_created_at, type",
        (merchant_id),
    )
    return [DirectMessage.from_row(row) for row in rows]
...
async def get_last_direct_messages_time(merchant_id: str) -> int:
    row = await db.fetchone(
        """
            SELECT time FROM customersupport.direct_messages 
...
async def get_last_direct_messages_created_at() -> int:
    row = await db.fetchone(
        """
            SELECT event_created_at FROM customersupport.direct_messages 
            ORDER BY event_created_at DESC LIMIT 1
        """,
        (),
    )
...
async def delete_merchant_direct_messages(merchant_id: str) -> None:
    await db.execute(
        "DELETE FROM customersupport.direct_messages WHERE merchant_id = ?",
        (merchant_id,),
    )
...
async def create_customer(merchant_id: str, data: Customer) -> Customer:
    await db.execute(
        f"""
        INSERT INTO customersupport.customers (merchant_id, public_key, meta)
        VALUES (?, ?, ?)
        """,
        (
            merchant_id,
...
async def get_customer(merchant_id: str, public_key: str) -> Optional[Customer]:
    row = await db.fetchone(
        "SELECT * FROM customersupport.customers WHERE merchant_id = ? AND public_key = ?",
        (
            merchant_id,
            public_key,
        ),
...
async def get_customers(merchant_id: str) -> List[Customer]:
    rows = await db.fetchall(
        "SELECT * FROM customersupport.customers WHERE merchant_id = ?", (merchant_id,)
    )
...
async def get_all_unique_customers() -> List[Customer]:
    q = """
            ...
            FROM customersupport.customers 
            GROUP BY public_key
        """
...
async def update_customer_profile(
    ...
    await db.execute(
        f"UPDATE customersupport.customers SET event_created_at = ?, meta = ? WHERE public_key = ?",
...
async def increment_customer_unread_messages(merchant_id: str, public_key: str):
    await db.execute(
        f"UPDATE customersupport.customers SET unread_messages = unread_messages + 1 WHERE merchant_id = ? AND public_key = ?",
        (
            merchant_id,
            public_key,
        ),
...
async def update_customer_no_unread_messages(merchant_id: str, public_key: str):
    await db.execute(
        f"UPDATE customersupport.customers SET unread_messages = 0 WHERE merchant_id =? AND public_key = ?",
        (
            merchant_id,
            public_key,
        ),
