    ...
    methods: {
      ...
      getOrders: async function () {
        try {
          ...
          const { data } = await LNbits.api.request(
            'GET',
            `/customersupport/api/v1/${ordersPath}?${query.join('&')}`,
            this.inkey
          )
      ...
      getOrder: async function (orderId) {
        try {
          const { data } = await LNbits.api.request(
            'GET',
            `/customersupport/api/v1/order/${orderId}`,
            this.inkey
          )
      ...
      restoreOrder: async function (eventId) {
        ...
        try {
          ...
          const {data} = await LNbits.api.request(
            'PUT',
            `/customersupport/api/v1/order/restore/${eventId}`,
            this.adminkey
          )
          await this.getOrders()
          this.$q.notify({
      ...
      restoreOrders: async function () {
        try {
          ...
          await LNbits.api.request(
            'PUT',
            `/customersupport/api/v1/orders/restore`,
            this.adminkey
          )
          await this.getOrders()
          this.$q.notify({
      ...
      reissueOrderInvoice: async function (order) {
        try {
          const { data } = await LNbits.api.request(
            'PUT',
            `/customersupport/api/v1/order/reissue`,
            this.adminkey,
            {
              id: order.id,
      ...
      updateOrderShipped: async function () {
        ...
        try {
          await LNbits.api.request(
            'PATCH',
            `/customersupport/api/v1/order/${this.selectedOrder.id}`,
            this.adminkey,
            {
      ...
      getStalls: async function (pending = false) {
        try {
          const { data } = await LNbits.api.request(
            'GET',
            `/customersupport/api/v1/stall?pending=${pending}`,
            this.inkey
          )
