    ...
    methods: {
      ...
      getStall: async function () {
        try {
          const { data } = await LNbits.api.request(
            'GET',
            '/customersupport/api/v1/stall/' + this.stallId,
            this.inkey
          )
      ...
      getProducts: async function (pending = false) {
        try {
          const { data } = await LNbits.api.request(
            'GET',
            `/customersupport/api/v1/stall/product/${this.stall.id}?pending=${pending}`,
            this.inkey
          )
          return data
        } catch (error) {
      ...
      updateProduct: async function (product) {
        try {
          const { data } = await LNbits.api.request(
            'PATCH',
            '/customersupport/api/v1/product/' + product.id,
            this.adminkey,
            product
          )
      ...
      createProduct: async function (payload) {
        try {
          const { data } = await LNbits.api.request(
            'POST',
            '/customersupport/api/v1/product',
            this.adminkey,
            payload
          )
          this.products.unshift(data)
        ...
        LNbits.utils
          ...
          .onOk(async () => {
            try {
              await LNbits.api.request(
                'DELETE',
                '/customersupport/api/v1/product/' + productId,
                this.adminkey
              )
